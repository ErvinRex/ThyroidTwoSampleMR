...
#samplesize.outcome <- read.csv(file = "PTSD", sep = "\t")[ ,c('SNP','Nca', 'Nco', 'P')]


#taking out the insignificant p values - anything <0.05
#out_dat <- out_dat[out_dat$pval.outcome < 0.05, ]

#summing the population sizes
#samplesize.outcome$samplesize.outcome <- rowSums(samplesize.outcome[,c("Nca", "Nco")])


#matching the snps to samplesize
#out_dat <- merge(out_dat, samplesize.outcome, by="SNP")
...
dat2$id.exposure <- replace(dat2$id.exposure, is.character(dat2$id.exposure), "TSH")
...
res_moe_filtered = subset(res_moe$TSH.PTSD$estimates, steiger_filtered== "TRUE")
res_moe_filtered = subset(res_moe_filtered, outlier_filtered == "TRUE")

#estimate MOE values
estimates <- res_moe$TSH.PTSD$estimates

#heterogeneity
heterogeneity <- res_moe$TSH.PTSD$heterogeneity

#directional pleiotropy
directional_pleiotropy <- res_moe$TSH.PTSD$directional_pleiotropy

#info
info <- res_moe$TSH.PTSD$info

#snps retained
snps_retained <- res_moe$TSH.PTSD$snps_retained

###Now we know which methods to use that are best suited to our data

# Perform MR ###no simple mean in TwoSampleMR
res <- mr(dat2, method_list = c("mr_weighted_mode", "mr_ivw"))
...
#dat100 <- dat2[1:20,]
res_single <- mr_singlesnp(dat2, all_method = c("mr_weighted_mode", "mr_ivw"))
...
#write.csv(exp_dat, "PTSD_CLUMPED")

out_dat <- read.csv("TSH_outcome", sep = ",")
...
dat2$id.outcome <- replace(dat2$id.outcome, is.character(dat2$id.outcome), "TSH")
...
#only steiger filtered
res_moe_filtered = subset(res_moe$PTSD.TSH$estimates, steiger_filtered== "TRUE")
res_moe_filtered = subset(res_moe_filtered, outlier_filtered == "TRUE")

#estimate MOE values
estimates <- res_moe$PTSD.TSH$estimates

#heterogeneity
heterogeneity <- res_moe$PTSD.TSH$heterogeneity

#directional pleiotropy
directional_pleiotropy <- res_moe$PTSD.TSH$directional_pleiotropy

#info
info <- res_moe$PTSD.TSH$info

#snps retained
snps_retained <- res_moe$PTSD.TSH$snps_retained

###Now we know which methods to use that are best suited to our data

# Perform MR
res <- mr(dat2, method_list = c("mr_weighted_median", "mr_ivw", "mr_simple_mode"))
...
#forest plots comparing MR values of each SNP
#dat100 <- dat2[1:20,]
res_single <- mr_singlesnp(dat2, all_method = c("mr_weighted_median", "mr_ivw", "mr_simple_mode"))
...
#leave one out analysis, MR but each SNP left out in turn - identifies if a single SNP is driving an association
res_loo <- mr_leaveoneout(dat2, method = mr_weighted_median)
#can change it with "method" argument - default is IVW
#change method to egger regression
